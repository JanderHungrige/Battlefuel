...
def _nearest_tanker(
    ...
    """The nearest fuelled FUEL_SUPPLY instance of the unit's fuel type (great-circle distance)."""
    ...
    if not candidates:
        return None
    return min(candidates, key=lambda t: haversine_m(unit.lon, unit.lat, t.lon, t.lat))
...
async def plan_move_with_refuel(
    ...
    *,
    ...
    """Stitch a refuel stop into a unit's move. ``None`` if no tanker / unroutable; ``LookupError``
    ...
    tanker = _nearest_tanker(unit, list(await instances.list_instances(session)), units, fuel_type)
    if tanker is None:
        return None
